<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            تسجيل دفعة للفاتورة
            {{ sprintf('%06d', $purchase->id) }}
            للمورد {{ $purchase->vendor->name }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class=" max-w-4xl bg-white shadow rounded p-3 mx-auto">
            <form action="{{ route('addPaymentPost', $purchase) }}" method="post">
                @csrf
                <div>
                    ...
                    <x-jet-input id="paidAmount" class="block mt-1 w-full" type="text" name="paidAmount"
                        value="{{ $purchase->totalDue() }}" required autofocus autocomplete="paidAmount" />
                </div>
                <div class="mt-4">
                ...
                <div class="mt-4">
                    <label for="currency_value">{{ __('قيمة العملة') }}</label>
                    <x-currency-input />
                </div>
