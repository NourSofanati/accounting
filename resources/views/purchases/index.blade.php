    ...
    <div class="p-6">
        ...
        <div class="flex gap-3 mb-4 ">
            <a href="{{ route('purchases.create') }}"
                class="px-3 py-4 text-white block rounded-xl cursor-pointer hover:scale-105 transition-all duration-100 hover:shadow-xl shadow-md font-bold bg-green-400">فاتورة
                شراء
                جديدة</a>
        </div>
        <div class="border-2 border-dashed p-4">
            <h1 class="text-2xl text-gray-700 mb-4">الأصول:</h1>
            @if ($purchases->where('type', 'asset')->count())
                <div class="flex flex-wrap gap-10">
                    @foreach ($purchases->where('type', 'asset') as $p)
        ...
        <div class="border-2 border-dashed p-4 mt-5">
            <h1 class="text-2xl text-gray-700 mb-4">المواد:</h1>
            @if ($purchases->where('type', 'material')->count())
                <div class="flex flex-wrap gap-10">
                    @foreach ($purchases->where('type', 'material') as $p)
                        <a href="{{ route('purchases.show', $p) }}"
                            class="bg-white border-2 hover:scale-105 hover:shadow-2xl transition-all duration-100 cursor-pointer">
                            <header class="p-2">
                                رقم الفاتورة: {{ sprintf('%06d', $p->id) }}
